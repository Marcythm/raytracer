...
public:
    ...
    ): origin(lookfrom), w(-lookdir.unit()), u(cross(viewup, w).unit()), v(cross(w, u)), lens_radius(aperture / 2) {
    ...
    constexpr Camera(
        ...
        const f64 vertical_field_of_view, const f64 aspect_ratio,
        const f64 aperture, const f64 focus_distance
    ): Camera(lookfrom, lookat - lookfrom, viewup, vertical_field_of_view, aspect_ratio, aperture, focus_distance) {}
